...
- 顶部仍保留当前价与涨跌摘要；右侧缩略图隐藏或弱化，避免与大图重复。
- 下方：时间段切换（沿用 `30天 / 90天 / 全部`）+ 大图折线。
- 大图下方居中向上箭头（如 `chevron.up`），用于收起。
...
- 复用 `market-trend.ts`（`filterTrend` / `plotTrend` / `trendStats` / `jobCopy`）；图表继续用现有 View 折线，不新增依赖、不改 API。
